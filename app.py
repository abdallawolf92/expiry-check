import pandas as pd import streamlit as st import sqlite3 from datetime import datetime, timedelta, timezone from PIL import Image import hashlib import socket import os
...
c.execute("SELECT COUNT(*) FROM users") user_count = c.fetchone()[0] if user_count == 0: c.execute("INSERT INTO users (username, password_hash) VALUES (?, ?)", ("admin", hashlib.sha256("2025".encode()).hexdigest())) conn.commit() st.success("✅ تم إنشاء حساب Admin تلقائي (admin/2025) عند أول تشغيل.")
...
c.execute("SELECT id, last_login, is_logged_in FROM users WHERE is_logged_in = 1") active_users = c.fetchall() for user in active_users: user_id, last_login, is_logged_in = user if last_login: last_login_time = datetime.strptime(last_login, "%Y-%m-%d %H:%M:%S") if datetime.now(baghdad_tz) - last_login_time.replace(tzinfo=baghdad_tz) > timedelta(seconds=30): c.execute("UPDATE users SET is_logged_in = 0 WHERE id = ?", (user_id,)) conn.commit()

if os.path.exists("logo.png"): logo = Image.open("logo.png") st.image(logo, width=120) st.markdown('<p style="font-size:36px; text-align:center; font-weight:bold;">Expiry Checker 🧪</p>', unsafe_allow_html=True)

st.markdown("## تسجيل الدخول") username = st.text_input("👤 اسم المستخدم:") password = st.text_input("🔑 كلمة المرور:", type="password")

ip_address = socket.gethostbyname(socket.gethostname())

if st.button("تسجيل الدخول"): if username and password: c.execute("SELECT id, password_hash, is_logged_in FROM users WHERE username = ?", (username,)) result = c.fetchone() if result: user_id, stored_hash, is_logged_in = result if hash_password(password) == stored_hash: if is_logged_in: st.error("❌ هذا الحساب مسجل دخول في مكان آخر.") else: current_time = datetime.now(baghdad_tz).strftime("%Y-%m-%d %H:%M:%S") c.execute("UPDATE users SET last_login = ?, is_logged_in = 1, ip_address = ? WHERE id = ?", (current_time, ip_address, user_id)) conn.commit() st.success("✅ تم تسجيل الدخول بنجاح.") st.session_state['logged_in'] = True st.session_state['username'] = username else: st.error("❌ كلمة المرور غير صحيحة.") else: st.error("❌ المستخدم غير موجود.") else: st.warning("⚠️ يرجى إدخال اسم المستخدم وكلمة المرور.")
...
if st.session_state.get('logged_in'): if os.path.exists(file_path): try: df = pd.read_excel(file_path) except Exception as e: st.error(f"❌ خطأ في قراءة الملف: {e}") st.stop()
...
if st.session_state.get('username') == 'admin': st.markdown("## 📊 لوحة التحكم") # تحديث: عرض جميع المستخدمين بما فيهم الذين لم يسجلوا دخول بعد user_stats = pd.read_sql_query("SELECT id, username, last_login, ip_address FROM users ORDER BY id ASC", conn) st.dataframe(user_stats) count_today = pd.read_sql_query("SELECT COUNT(*) as count FROM users WHERE DATE(last_login) = DATE('now', 'localtime')", conn)['count'][0] st.info(f"✅ عدد المستخدمين الذين دخلوا اليوم: {count_today}")
